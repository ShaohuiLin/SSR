...
tf.app.flags.DEFINE_string(
    'model_name', 'lenet_aulm', 'The name of the architecture to train.')
...
tf.app.flags.DEFINE_integer('max_number_of_steps', 8000,
...
tf.app.flags.DEFINE_integer(
    'update_fy_step', 200,
    'per step to update F and Y')

tf.app.flags.DEFINE_string(
    'update_layer_name', None,
    'aulm update layer name')
...
def _get_variables_to_train():
    ...
    """
    if FLAGS.update_layer_name is None:
        ...
        for var in tf.trainable_variables():
            if FLAGS.update_layer_name in var.name:
...
def train_step(sess, train_op, global_step, train_step_kwargs):
    ...
    if np_global_step % FLAGS.update_fy_step == 0:
        update_FY = tf.get_collection("UPDATE_FY")
        sess.run(update_FY)
    ...
    if 'should_log' in train_step_kwargs:
        if sess.run(train_step_kwargs['should_log']):
            compress = tf.get_collection("COMPRESS")
            compress_value = sess.run(compress)
            logging.info('global step %d: loss = %.4f, compress = %.2f (%.3f sec/step)',
                   np_global_step, total_loss, compress_value[0], time_elapsed)
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        # Add summaries for compress.
        for compress in tf.get_collection("COMPRESS"):
            summaries.add(tf.summary.scalar('compress/%s' % compress.op.name, compress))
        ...
        # with tf.control_dependencies([update_FY]):
